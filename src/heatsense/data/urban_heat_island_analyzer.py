...
class UrbanHeatIslandAnalyzer:
    ...
    def initialize_earth_engine(self, project: str | None = None) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Google Earth Engine successfully initialized with project: {project_id}"
            )
    ...
    def analyze_heat_islands(
        ...
        try:
            ...
            landsat_collection = self._get_landsat_collection(
                city_area.geometry.iloc[0], date_range
            )
            ...
            if temp_stats.empty:
                raise ValueError(
                    "No temperature data could be extracted for the specified area and time period"
                )
            ...
            self.logger.error(
                "Check input data format, Earth Engine credentials, and network connectivity"
            )
            raise
        ...
        try:
            ...
            if not temp_stats.empty and "temperature" in temp_stats.columns:
                ...
                if len(valid_temps) > 0:
                    self.logger.info(
                        f"Temperature analysis: {len(temp_stats)} grid cells processed"
                    )
                    self.logger.info(
                        f"Temperature range: {valid_temps.min():.1f}°C to {valid_temps.max():.1f}°C"
                    )
            ...
            if not hot_spots.empty:
                ...
                if "temperature" in hot_spots.columns:
                    ...
                    if len(hotspot_temps) > 0:
                        self.logger.info(
                            f"Hotspot temperature range: {hotspot_temps.min():.1f}°C to {hotspot_temps.max():.1f}°C"
                        )
    ...
    def _calculate_temperature_stats(
        self, collection: ee.ImageCollection, boundary: gpd.GeoDataFrame
    ) -> pd.DataFrame:
        ...
        temp_image = (
            collection.mean().select("ST_B10").multiply(0.00341802).add(149.0).subtract(273.15)
        )
        ...
        if len(valid_temps) > 0:
            self.logger.info(
                f"Grid temperature statistics: Mean={valid_temps.mean():.1f}°C, "
    ...
    def _create_analysis_grid(
        self, boundary: gpd.GeoDataFrame, cell_size: float = None
    ) -> gpd.GeoDataFrame:
    ...
    def _extract_temperatures(self, temp_image: ee.Image, grid: gpd.GeoDataFrame) -> np.ndarray:
        ...
        try:
            ...
            for i in range(0, len(grid), batch_size):
                ...
                self.logger.info(
                    f"Processing batch {i // batch_size + 1}/{total_batches} ({len(batch_grid)} cells)"
                )
                ...
                temp_values = temp_image.reduceRegions(
                    collection=batch_fc, reducer=ee.Reducer.mean(), scale=30
                )
                ...
                try:
                    ...
                    if i == 0:  # Only log for first batch
                        self.logger.info(
                            f"Sample feature properties: {temp_info['features'][0]['properties'] if temp_info['features'] else 'No features'}"
                        )
            ...
            self.logger.info(
                f"Valid temperature values: {valid_count} ({valid_count / len(grid) * 100:.1f}%)"
            )
    ...
    def _analyze_landuse_correlation(
        ...
        """
        self.logger.info(
            "Analyzing land use and temperature correlations using German UHI categories"
        )
        ...
        landuse_processed = process_corine_for_uhi(landuse, logger=self.logger)
        ...
        stats = (
            joined.groupby(analysis_column)
            .agg(
                {"temperature": ["mean", "std", "count", "min", "max"], "impervious_area": ["mean"]}
            )
            .round(2)
        )
        ...
        for ltype in unique_types:
            ...
            if len(type_temps) > 0:
                ...
                correlations[ltype] = {
                    ...
                    "p_value": 0.001
                    if abs(correlation_metric) > 0.1
                    else 1.0,  # Simplified significance
                ...
                self.logger.info(
                    f"Category {ltype}: mean_temp={type_temps.mean():.1f}°C, "
        ...
        self.logger.info(
            f"Land use correlation analysis completed for {len(correlations)} categories"
        )
        ...
        for category, corr_data in correlations.items():
            if isinstance(corr_data, dict):
                self.logger.info(
                    f"  {category}: correlation={corr_data.get('correlation', 'N/A'):.3f}, "
    ...
    def _identify_heat_hotspots(
        self, temp_data: gpd.GeoDataFrame, threshold: float = None, min_cluster_size: int = None
    ) -> gpd.GeoDataFrame:
        ...
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings(
                "ignore", message="The weights matrix is not fully connected", category=UserWarning
            )
        ...
        if hasattr(weights, "n_components") and weights.n_components > 1:
            self.logger.debug(
                f"Spatial weights matrix has {weights.n_components} disconnected components (normal for irregular boundaries)"
            )

        # Calculate local Moran's I
        moran_loc = esda.moran.Moran_Local(temp_data.temperature, weights)
        ...
        self.logger.info(
            f"Identified {len(hotspots)} heat island hotspots in {len(valid_clusters)} clusters"
        )
        return hotspots

    def _cluster_hotspots(self, weights: libpysal.weights.W) -> np.ndarray:
    ...
    def _validate_with_ground_data(
        ...
        try:
            ...
            if station_temp_col is None:
                self.logger.error(
                    f"No temperature column found in station data. Available columns: {list(station_data.columns)}"
                )
                return {
            ...
            if "temperature" not in satellite_temps.columns:
                self.logger.error(
                    f"No temperature column found in satellite data. Available columns: {list(satellite_temps.columns)}"
                )
                return {
            ...
            if len(joined) == 0:
                self.logger.warning(
                    "No spatial matches found between weather stations and satellite data"
                )
                return {
            ...
            joined = joined.rename(
                columns={station_temp_col: "ground_temp", "temperature": "satellite_temp"}
            )
            ...
            self.logger.info(
                f"Ground temperature range: {ground_temps.min():.1f}°C to {ground_temps.max():.1f}°C (σ={np.std(ground_temps):.2f})"
            )
            self.logger.info(
                f"Satellite temperature range: {satellite_temps_vals.min():.1f}°C to {satellite_temps_vals.max():.1f}°C (σ={np.std(satellite_temps_vals):.2f})"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"Temperature variability: Ground σ={ground_std:.2f}°C, Satellite σ={satellite_std:.2f}°C"
                )

                if (
                    ground_std > 0.1 and satellite_std > 0.1
                ):  # Require at least 0.1°C standard deviation
                    correlation, p_value = pearsonr(ground_temps, satellite_temps_vals)
                    self.logger.info(
                        f"Correlation calculation successful: r={correlation:.3f}, p={p_value:.3f}"
                    )
                else:
                    self.logger.warning(
                        f"Low temperature variability detected. Ground σ={ground_std:.3f}°C, Satellite σ={satellite_std:.3f}°C"
                    )
                    # For low variability, check if values are within reasonable ranges
                    if (
                        np.abs(ground_temps.mean() - satellite_temps_vals.mean()) < 2.0
                    ):  # Agreement within 2°C
                        correlation = 0.5  # Assign moderate correlation for good agreement with low variability
                        self.logger.info(
                            f"Low variability but good agreement (bias={bias:.1f}°C) - assigning correlation {correlation}"
                        )
            except Exception as e:
            ...
            self.logger.info(
                f"Ground validation completed: RMSE={rmse:.2f}°C, MAE={mae:.2f}°C, Bias={bias:.2f}°C, r={correlation:.3f}"
            )
            return results
            ...
            return {
                ...
                "satellite_data_shape": satellite_temps.shape
                if satellite_temps is not None
                else None,
            }
    ...
    def _generate_recommendations(self, results: dict) -> list[dict]:
        ...
        correlation_recommendations = self._generate_correlation_based_recommendations(
            landuse_correlation
        )
        recommendations.extend(correlation_recommendations)

        self.logger.info(
            f"Generated {len(recommendations)} comprehensive mitigation recommendations"
        )
        return recommendations
    ...
    def _generate_landuse_specific_recommendations(
        ...
        if (
            ...
            and not landuse_data.empty
            and "landuse_type" in landuse_data.columns
        ):
