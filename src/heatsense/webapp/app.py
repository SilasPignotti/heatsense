...
def analyze():
    ...
    try:
        ...
        if performance_mode not in UHI_PERFORMANCE_MODES:
            return jsonify(
                {"status": "error", "errors": [f"Invalid performance mode: {performance_mode}"]}
            ), 400
        ...
        try:
            ...
            return jsonify(
                {"status": "error", "errors": [f"Invalid date format. Use DD.MM.YYYY: {str(e)}"]}
            ), 400
        ...
        logger.info(
            f"Starting UHI analysis: {area}, {start_date_parsed} to {end_date_parsed}, mode: {performance_mode}"
        )
        ...
        return jsonify(
            {"status": "error", "errors": [f"Analysis failed: {str(e)}"], "execution_time": 0}
        ), 500
...
def download_results():
    ...
    try:
        ...
        if not analysis_id:
            return jsonify(
                {"status": "error", "errors": ["No analysis results available for download"]}
            ), 404
        ...
        return jsonify({"status": "error", "errors": [f"Download failed: {str(e)}"]}), 500
